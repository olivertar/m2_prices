 ...
 * 2. It synchronously calls the original `this._super()` to allow Magento to fully initialize the widget and set up its DOM selectors, events, and internal caches.
 * 3. It hooks into the globally available `window.b2bPricesPromise` (provided by `b2b-prices-core.js`).
 * 4. When the promise resolves with the custom B2B prices, it modifies BOTH the widget's options (`self.options.prices`) AND its internal render cache (`self.cache.displayPrices`) 
 *    with the loaded B2B price for this specific widget instance.
                ...
                if (boxProductId && pricesById && pricesById[boxProductId]) {
                    ...
                    var updateAmount = function (obj) {
                        ...
                        for (var i = 0; i < singlePriceKeys.length; i++) {
                            if (obj[singlePriceKeys[i]]) {
                                obj[singlePriceKeys[i]].amount = b2bPriceData.final_price;
                            }
                        }

                        if (obj.minPrice && b2bPriceData.min_price !== undefined) {
                            obj.minPrice.amount = b2bPriceData.min_price;
                        }
                        if (obj.maxPrice && b2bPriceData.max_price !== undefined) {
